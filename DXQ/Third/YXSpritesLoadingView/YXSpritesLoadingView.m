...
{
    static YXSpritesLoadingView *sharedInstance = nil;
    @synchronized(self) {
        if (!sharedInstance) {
    ...
    if (!loadingImageView)
    {
        ...
        if (text && ![text isEqualToString:@""]) {
            ...
            if (shimmering) {
                ...
                loadingLabel = [[UIButton alloc]initWithFrame:shimmeringView.bounds];
            ...
            {
                loadingLabel = [[UIButton alloc]initWithFrame:CGRectMake(loadingTextLabelSideMargin,
                                                                        loaderView.frame.size.height - height - loadingTextLabelBottomMargin,
                                                                        loaderView.frame.size.width - (loadingTextLabelSideMargin * 2),
                                                                        height)];
            ...
            [loadingLabel setImage:[UIImage imageNamed:@"01-12-13"] forState:UIControlStateNormal];
            loadingLabel.imageView.image = [UIImage imageNamed:@"01-12-13"];
            loadingLabel.titleLabel.font = [UIFont fontWithName:loadingTextFontName size:loadingTextFontSize];
            loadingLabel.titleLabel.textAlignment = NSTextAlignmentCenter;
//            loadingLabel.titleLabel.text = text;
            loadingLabel.titleLabel.textColor = [UIColor whiteColor];//loadingTextColor;
            loadingLabel.titleLabel.numberOfLines = 0;
        }
        else
        {
...
- (NSArray *)imagesForAnimating
{
    ...
    [retVal addObject:[UIImage imageNamed:@"01-10"]];
    [retVal addObject:[UIImage imageNamed:@"01-11"]];
    return retVal;
}
