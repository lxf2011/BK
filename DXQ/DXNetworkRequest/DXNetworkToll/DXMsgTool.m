...
+ (void)getMsgByCatWithParams:(DXMsgListParam *)param success:(void (^)(DXMsgListModel *result))success failure:(void(^)(NSError *error))failure{
    NSString *url = [NSString stringWithFormat:@"%@/getMsgByCat",DXNetworkAddress];
    [self getWithUrl:url param:param resultClass:[DXMsgListModel class] success:success failure:failure];
}
